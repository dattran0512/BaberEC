...
<body>
    ...
    <!-- Header Area End -->
      ...
      <select id="selectThanhPho" name="type" onchange="selectTP(this)">
	  	  <option value="st1" >Hiển thị tất cả quận huyện</option>
	...
	<div class="table-inline"> 
			<a type="button" href="AdminAddDistrict2" class="btn btn-primary">Thêm</a>	
			 <table class="table table-hover">	
				      <thead>
					   ...
					   <c:forEach items="${listQuanHuyen}" var="x">
					   		<tr>
					   			<td> ${x.name}  </td> 	
								 <td>   <a href="AdminDeleteDistrict?idDistrict=${x.id}" > Xóa </a>  </td>
					   		</tr>
					   </c:forEach>
					   </tbody>		
					  </table>
...
<script>
	...
	document.getElementById('selectThanhPho').value=sessionStorage.getItem("idTP");
		function selectTP(selectObject){
			...
			window.location.href='AdminGetDistrictByCityID?idTP='+id;
			}else{
				sessionStorage.setItem("idTP",'st1');
				window.location.href='AdminGetAllDistrict'
			}
